        'use strict';
       // In-memory counters for each convo type
        const unreadCounts = {
            chatgpt: 0,
            public:  0,
            private: 0
        ...
        /**
          * Bump the unread counter, update the badge text & activate it.
          * @param {'chatgpt'|'public'|'private'} convo 
          * @param {string} fromName  // optional: you can show a tooltip later
        */
        function notifyUnread(convo, fromName) {
            unreadCounts[convo]++;
            const badge = document.getElementById(`badge-${convo}`);
            if (!badge) return;
            badge.innerText = unreadCounts[convo];
            badge.classList.add('active');
        // (optional) badge.title = `New from ${fromName}`;
        }



        /**
         * Pixselchat Lounge - Client component
         *
         */
        ...
        class RoomClient {
            ...
            appendMessage(side, img, fromName, fromId, msg, toId, toName) {
                ...
                switch (getToId) {
                    console.log('[newMessage] getToId=', getToId);
                    case 'ChatGPT':
                        chatGPTMessages.insertAdjacentHTML('beforeend', newMessageHTML);
                        notifyUnread('chatgpt', getFromName);
                        break;
                    case 'all':
                        chatPublicMessages.insertAdjacentHTML('beforeend', newMessageHTML);
                        notifyUnread('public', getFromName);
                        break;
                    default:
                        chatPrivateMessages.insertAdjacentHTML('beforeend', newMessageHTML);
                        notifyUnread('private', getFromName);
                        break;
                }

        function getRecipientLanguage() {
                    const contactStatus = localStorage.getItem('language').trim(); //document.getElementById('contactStatus').textContent.trim();
                    ...
                    return languageMapping[contactStatus] || 'en'; // Default to English if not found
                }
                ...
                if (message) {
                    if (getFromName === 'ChatGPT') {
                        ...
                        const targetLanguage = getRecipientLanguage();
                        translateMessage2(this.processMessage(getMsg), targetLanguage /*"en"*/, localStorage.getItem('voiceLang') /*"de"*/);

                        let translatedText;

                function translateMessage2(text, sourceLang, targetLang) {
                    console.log("Translating message from", sourceLang, "to", targetLang);

                    fetch('https://localhost/translate/', {  
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        if (data && data.translated_text) {
                            ...
                            console.log("translated msg value is " + translatedText);

                            displayMessage2(translatedText, 'sent'); 
                            //speakTextToAudio(translatedText, targetLang);
                            //message = translatedText;

                        } else {
                            console.error("Translation failed or response was invalid.");
                        }
                    })
                    .catch(error => console.error('Error in translation API:', error));
                }

                // Function to call the /synthesize endpoint
            function speakTextToAudio(text, language) {
                console.log("Synthesizing message:", text, "in language:", language);

                fetch('https://localhost/synthesize/', {
                    method: 'POST',
                    headers: {
                        'accept': 'application/json',
                        'Content-Type': 'application/json'
                ...
                .then(audioBlob => {
                    ...
                    playChatAudioMessage(audioUrl, 'my-message');
                })
                .catch(error => console.error('Error in synthesis API:', error));
            }

            // Function to transcribe audio files using the /transcribe endpoint
            function transcribeAudioToText(audioUrl) {
                console.log("Transcribing audio:", audioUrl); // Log when transcription starts
                ...
                .then(audioBlob => {
                    ...
                    // console.log("FormData created, sending to /transcribe");
            
                    // Send the audio Blob to the /transcribe endpoint
                    return fetch('https://localhost/transcribe?target_language=en', {
                        method: 'POST',
                        body: formData
                    });
                })
                ...
                .then(data => {
                    ...
                    if (data && data.transcription) {
                        //displayMessage2(data.transcription, 'received');  // Display the transcribed text as a chat message
                    } else {
                        console.error("Transcription failed or response was invalid.");
                    }
                })
                .catch(error => console.error('Error in transcription API:', error));
            }
            
            // Function to display audio messages in the chat
            function playChatAudioMessage(audioUrl, type) {
            ...
            function displayMessage2(messageText, type) {
                
                //socket.emit("privateMessage", { targetId: selectedTargetId, message: messageText });
                 
                  //const msgElem = document.createElement('div');
                    //msgElem.classList.add('message', 'sent');
                    message.innerHTML = `<p>${messageText}</p>`;
                    //document.getElementById("chatHistory").appendChild(message);
                    //messageText.value = '';
                    //document.getElementById("chatHistory").scrollTop = document.getElementById("chatHistory").scrollHeight;
                 // console.log("Sending message to:", selectedTargetId, "Message:", messageText);
            }
            ...
            formatMsg(msg) {
                ...
                //translateMessage2(message, "en", "de");
        ...
        let translatedText;
            ...
            speechElementText(elemId) {
                ...
                const targetLanguage = languageMapping[contactStatus] || 'en';
                speakTextToAudio(element.innerText, targetLanguage);
            }
        ...
        // Function to call the /synthesize endpoint
            ...
            const audioElement = document.createElement('audio');
            audioElement.controls = true;
            audioElement.autoplay = true; // Enables autoplay
            audioElement.src = audioUrl;

            // Try to play the audio explicitly (to handle autoplay restrictions)
            audioElement.play().catch(error => {
                console.log('Autoplay failed:', error);
            });
        }
        // ─── Clear unread badges when the user opens the chat panel ───
            document.addEventListener('DOMContentLoaded', () => {
             const btn = document.getElementById('participantsUnreadMessagesBtn');
             if (!btn) return;
             btn.addEventListener('click', () => {
                ['chatgpt', 'public', 'private'].forEach((convo) => {
                    // reset our in-memory counters
                    unreadCounts[convo] = 0;
                    // clear & hide the badge
                    const b = document.getElementById(`badge-${convo}`);
                    if (b) {
                        b.innerText = '';
